...
class BaiduSpider(scrapy.Spider):
    ...
    for word in words:
        # 该url没有出现重定向的的情况（302）
        url = "http://www.baidu.com/s?q=&tn=baiduhome_pg&ct=0&si=&ie=utf-8&cl=3&wd=%s" % urllib.parse.quote(word)  #将word编码
        print(url)
    ...
    def parse(self, response):
        ...
        for t in response.xpath('//div[@id="content_left"]/div'):
            ...
            if t.xpath('./@id').extract() in a:
                item['title'] = (''.join(t.xpath('./h3/a//text()').extract()).strip())
                # print items['title'].encode('utf-8')
                item['url'] = t.xpath('./h3/a/@href').extract()[0]
                item['content'] = (' '.join(pattern.findall(''.join(t.xpath('./div[1]//text()').extract()))))
                #print item['content']
